#!/usr/bin/env python3
"""
Model B — LightGBM with Optuna hyperparameter search.
Trains one model per affordance on structured features (~310 dims).
"""
...
FEATURE_NAMES_PATH = DATA / "assembled_dataset" / "feature_names.json"
...
# Raw feature groups (no indicator columns for Model B)
RAW_FEATURE_PREFIXES = ("presence_", "count_", "dist_", "depth_diff_",
...
def train_affordance(
    ...
    # Combine train+val for the Optuna search
    ...
    print(f"  Best CV RMSE: {study.best_value:.4f} ({elapsed_search:.0f}s, {N_TRIALS} trials)")
    ...
    model_path = aff_dir / "lgbm_model.pkl"
    with open(model_path, "wb") as f:
        pickle.dump(final_model, f)

    study_path = aff_dir / "optuna_study.pkl"
    with open(study_path, "wb") as f:
        pickle.dump(study, f)

    params_path = aff_dir / "best_params.json"
...
def main() -> None:
    t0 = time.time()
    print("=" * 60)
    print("Model B — LightGBM Training")
    print("=" * 60)

    OUT_MODELS_B = OUT_MODELS / "lgbm"
    OUT_MODELS_B.mkdir(parents=True, exist_ok=True)
    ...
    feature_cols = get_raw_feature_cols(df)
    ...
    for aff_id in AFFORDANCES:
        ...
        result = train_affordance(
            ...
            out_dir=OUT_MODELS_B,
        )
        all_results.append(result)

    # Save summary
    summary_path = OUT_RESULTS / "lgbm_training_summary.json"
    with open(summary_path, "w") as f:
        json.dump(all_results, f, indent=2)
    print(f"\nSaved summary: {summary_path}")

    # Print table
    print("\nModel B — Test Results Summary")
    print(f"{'Affordance':<10} {'RMSE':>8} {'MAE':>8} {'Pearson r':>10} {'Spearman ρ':>11}")
    print("-" * 52)
    for r in all_results:
        m = r["test_metrics"]
        print(f"{r['affordance_id']:<10} {m['rmse']:>8.4f} {m['mae']:>8.4f} "
              f"{m['pearson_r']:>10.4f} {m['spearman_rho']:>11.4f}")
